...
import 'package:nae/screens/wh/return/edit_fullscreen.dart';
...
class WHReturn extends Entity {
  ...
  @override
  String name() => "return to the warehouse";

  @override
  IconData icon() => Icons.keyboard_return;
  ...
  Widget screen(String action, MemoryItem entity) {
    return EntityScreens(
        ...
        list: ScaffoldListCalendar(
          entityType: WHReturn.ctx,
          newBtn: (context) {
            context.read<UiBloc>().add(ChangeView(WHReturn.ctx,
                action: 'edit', entity: MemoryItem.create()));
          },
          newBtnTooltip: (context) => AppLocalizations.of(context)
              .translate("new return to the warehouse"),
          ...
          listBuilder: (date) => WHReturnListBuilder(date: date),
        ),
        view: action == 'view'
            ? WHReturnEditFS(
            ...
            : WHReturnDocumentCreation(entity: entity));
  }
}

class WHReturnListBuilder extends StatelessWidget {
  const WHReturnListBuilder({super.key, required this.date});
