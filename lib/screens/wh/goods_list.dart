...
class WHGoodsList extends StatefulWidget {
  const WHGoodsList({Key? key}) : super(key: key);

  @override
  State<WHGoodsList> createState() => _WHGoodsListState();
}

class _WHGoodsListState extends State<WHGoodsList> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_first) {
      ...
      Api.feathers().listen(serviceName: "memories", fromJson: (e) {
        print("event "+e);
        // getFromServer(settings);
      });
      _first = false;
  ...
  void getFromServer(MySettings settings) async {
    try {
      ...
      Api.feathers().find(serviceName: "memories", query: query).asStream().listen((event) {
        setState(() {
          rows = event["data"];
        });
        print(rows);
